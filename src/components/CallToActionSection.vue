<script setup>
</script>

<template>
  <div id="calltoaction-section"
    class="w-full xl:h-screen flex justify-center items-center relative z-10 px-[12px] xl:px-0 mb-[60px] xl:mb-0" data-aos="fade-up" data-aos-delay="0">
    <div
      class="xl:w-[1227px] h-[480px] rounded-[15px] flex flex-col gap-[80px] items-start justify-center bg-[url(/assets/cta-bg.png)] px-[56px] relative">
      <img src="/assets/laptop.png" alt="" class="absolute right-0 hidden xl:inline" />
      <div class="flex flex-col gap-[24px] xl:w-[704px]">
        <h4 class="text-[20px] xl:text-[42px] font-goldman font-bold">
          Bangun website profesional untuk bisnismu hari ini!
        </h4>
        <p class="text-[14px] xl:text-[20px] font-medium">
          Website cepat, responsif, dan siap pakai hanya dalam hitungan hari.
          Konsultasi gratis sekarang!
        </p>
      </div>
      <button class="px-[38px] py-[10px] rounded-[10px] bg-gradient-to-b from-grad-1 to-grad-2 text-[20px] font-bold">
        <a href="https://wa.me/628174745070">Konsultasi gratis</a>
      </button>
    </div>
  </div>
</template>
